...
<body>
	...
	function onGstNo() {
			...
			if(gstValue !=null && gstValue!=" " && gstValue.length>3 && gstValue.length<30) 
				...
				xhttp.open("GET","http://localhost:8080/vendorManagement/uniqueGst/"+gstValue);
				xhttp.send();
		...
		function onContact()
		{
			...
			if(contactValue != "" && contactValue.length == 10) 
				...
				const xhttp=new XMLHttpRequest();
				xhttp.open("GET","http://localhost:8080/vendorManagement/uniqueContact/"+contactValue);
				xhttp.send();
				xhttp.onload=function()
				{
					console.log(this);
		...
		function onEmail() {
			...
			if(mailsValue !=null && mailsValue!=" " && mailsValue.length>3 && mailsValue.length<30) 
				...
				const xhttp=new XMLHttpRequest();
				xhttp.open("GET","http://localhost:8080/vendorManagement/uniqueEmail/"+mailsValue);
				xhttp.send();
				xhttp.onload=function()
				{
					console.log(this);
		...
		function onWebsite()
		{
			...
			if(webValue !=null && webValue!=" " && webValue.length>3 && webValue.length<30) 
				...
				const xhttp=new XMLHttpRequest();
				xhttp.open("GET","http://localhost:8080/vendorManagement/uniqueWebsite/"+webValue);
				xhttp.send();
				xhttp.onload=function()
				{
					console.log(this);
